...
def check_rwkv_info(state_dict):
    ...
    possible_head_keys = ['head.weight', 'lm_head.weight', 'output.weight']
    
    for key in possible_head_keys:
        if key in state_dict:
            head_weight_key = key
            break
    
    if head_weight_key is None:
        # 如果找不到标准的头部权重，尝试从其他层推断
        print(f"Warning: Could not find head weight. Available keys: {list(state_dict.keys())[:10]}...")
        # 尝试从嵌入层推断
        if 'emb.weight' in state_dict:
            n_vocab, n_embd = state_dict['emb.weight'].shape
        else:
            # 从第一个块的权重推断嵌入维度
            for k in state_dict.keys():
                if 'blocks.0.' in k and '.weight' in k:
                    if len(state_dict[k].shape) >= 2:
                        n_embd = state_dict[k].shape[-1]
                        break
            # 设置一个默认的词汇表大小
            n_vocab = 65536  # 常见的RWKV词汇表大小
    else:
        n_vocab, n_embd = state_dict[head_weight_key].shape
    for k in state_dict.keys():
